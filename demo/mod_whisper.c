...
static void whisper_speech_text_param_tts(switch_speech_handle_t *sh, char *param, const char *val)
{
	...
	if (!zstr(param) && !zstr(val)) {
		if (!strcasecmp("channel-uuid", param)) {
			...
			switch_log_printf(SWITCH_CHANNEL_UUID_LOG(context->channel_uuid), SWITCH_LOG_DEBUG, "channel-uuid = %s\n", val);
		} else if (!strcasecmp("tts-engine", param)) {
			context->engine = switch_core_strdup(sh->memory_pool, val);
			switch_log_printf(SWITCH_CHANNEL_UUID_LOG(context->channel_uuid), SWITCH_LOG_DEBUG, "tts-engine = %s\n", val);
		} else if (!strcasecmp("tts-voice", param)) {
			context->voice = switch_core_strdup(sh->memory_pool, val);
			switch_log_printf(SWITCH_CHANNEL_UUID_LOG(context->channel_uuid), SWITCH_LOG_DEBUG, "tts-voice = %s\n", val);
		} else if (!strcasecmp("tts-server-url", param)) {
			context->tts_server_url = switch_core_strdup(sh->memory_pool, val);
			switch_log_printf(SWITCH_CHANNEL_UUID_LOG(context->channel_uuid), SWITCH_LOG_DEBUG, "tts-server-url = %s\n", val);
		}
	}
}
